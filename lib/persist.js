...
export function snapshot(state, showFile) {
  return {
    ...
    // Who was driving. Restored deliberately: if their console has not come back yet
    // the result is the empty-chair warning, which is the honest state of the room.
    con: state.con,
    ...
    // Ad-hoc timers are not in the show file, so they have to travel in the snapshot
    // or a crash during a break would come back with no break and no way home.
    adhoc: state.sessions.filter((s) => s.adhoc),
  };
}
...
export function restore(state, snap) {
  ...
  if (snap.con) state.con = snap.con;
  if (snap.published) state.published = snap.published;

  // Put any ad-hoc timer back before anything else looks at the rundown, anchored to
  // the session it suspended so it lands where it actually happened.
  for (const a of snap.adhoc || []) {
    const anchor = a.resume?.sessionId;
    const at = anchor ? state.sessions.findIndex((x) => x.id === anchor) : 0;
    state.sessions.splice(at >= 0 ? at : 0, 0, a);
  }
