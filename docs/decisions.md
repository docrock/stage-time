...
- 2026-09-22 — Public-facing outputs hold a PUBLISHED schedule rather than following live projection. Doc's point: a rundown is padded on purpose, and when a segment stretches the producer absorbs it by trimming a host-led block or pulling a promo reel. Pushing internal drift onto an audience screen announces a delay that is about to be absorbed, and announces it repeatedly. Republishing is a deliberate human act. `timer.publicFollowsLive` opts out, and `?times=live|published` pins any single output either way.
- 2026-09-22 — Added `shows/demo-creator-summit.json`, a fully fictional rundown, as the thing testers use. It carries realistic padding and two hard stops so the interesting failure modes can actually be exercised.
- 2026-09-22 — `docs/SHARING.md` records the access decision: the repo history contains real vendor contacts and a client run of show, so deleting files does not unshare them. Three options written up. NOT actioned, because who gets access is Doc's call.
